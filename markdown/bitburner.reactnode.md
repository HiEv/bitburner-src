...
A stand-in for the real `React.ReactNode`. A [ReactElement](./bitburner.reactelement.md) is rendered dynamically with React. ReactNodes of `number` and `string` types are displayed directly, while `boolean`, `null`, and `undefined` types are ignored and not rendered. An array of ReactNodes will display all members of that array sequentially.
